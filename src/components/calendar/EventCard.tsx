...
import { ResizeHandles } from "./calendar-components";
  ...
  return (
    <Button
      ...
      {canResize && onResizeStart && (
        <ResizeHandles
          onResizeStart={(edge, e) => onResizeStart(edge, e)}
        />
      )}
      ...
      {layout.height > 50 && event.metadata?.location && renderLocation?.(event, styleInfo.color)}
    </Button>
  );
};
